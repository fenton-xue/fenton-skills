...
使用 [scripts/parse-testcase.mjs](scripts/parse-testcase.mjs) 解析固定字段和编号关系：

```js
var fsTestcase = await import("node:fs/promises");
var { parseTestcases } = await import(
  "<testcase-executor目录>/scripts/parse-testcase.mjs"
);
var sourceText = await fsTestcase.readFile("<测试用例文件>", "utf8");
var parsedTestcases = parseTestcases(sourceText);
```

## 准备执行计划

1. 将模块字段转换为有序的 `module_path`。
2. 为每条用例保留 `precondition`；源用例未提供时使用 `null`，并将该字段放在 `steps` 正前方。
3. 从项目上下文、文件路径或用户指令确定 `system`。
4. 使用模块路径查找项目页面知识：
