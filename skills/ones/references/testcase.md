...
- `steps: list[dict]`，必填。由 SKILL 把本地步骤描述/预期结果字段整理成数组；每步至少包含：
  ...
  - 如果单个步骤描述或预期结果本身包含多行或编号列表，把这些行拼成同一个字符串，行与行之间使用 `\n`；字符串开头不要加 `\n`。
...
转换为 `upsert_test_case` 入参：

```json
{
  "library_id": "8WeKJafQ",
  "module_path": ["新版QCAPP", "日志", "离线队列与上传"],
  "name": "#84358-02-测试",
  "steps": [
    {
      "description": "1. 测试步骤描述1-1\n2. 测试步骤描述1-2",
      "expected_result": "1. 测试预期结果1-1\n2. 测试预期结果1-2"
    },
    {
      "description": "测试步骤描述2",
      "expected_result": "测试预期结果4"
    }
  ]
}
```
